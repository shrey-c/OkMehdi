...
def searchCurrency(country):
    for i in range(len(allcurrencies)):
        if allcurrencies[i]['name'].lower() == country:
            return allcurrencies[i]['code']
    return "Error"

def convert(value, from_country, to_country):
    print(from_country)
    print(to_country)
    try:
        # Where USD is the base currency you want to use
        url = 'https://api.exchangerate-api.com/v4/latest/' + from_country
        ...
        # Your JSON object
        return str("%.2f" % (data['rates'][to_country] * value))
    except:
